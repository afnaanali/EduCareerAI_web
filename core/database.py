...
def save_student_profile(user_id: int, profile_data: Dict[str, Any]) -> bool:
    ...
    hobbies = {k: v for k, v in profile_data.items() if k.startswith("Hobby_") or k.startswith("H_")}
    grades = {k: v for k, v in profile_data.items() if k.startswith("Grade_") or k.startswith("G_")}
    aptitudes = {k: v for k, v in profile_data.items() if k.startswith("Aptitude_") or k.startswith("A_")}
...
def load_student_profile(user_id: int) -> Optional[Dict[str, Any]]:
    ...
    profile = {"field": row["target_field"]}
    if row["hobbies_json"]:
        profile.update(json.loads(row["hobbies_json"]))
    if row["grades_json"]:
        profile.update(json.loads(row["grades_json"]))
    if row["aptitudes_json"]:
        profile.update(json.loads(row["aptitudes_json"]))
    return profile
