...
def create_career_features(student_dict: dict) -> pd.DataFrame:
    ...
    for h in HOBBY_COLUMNS:
        clean_dict[h] = float(student_dict.get(h, 0))
    for g in GRADE_COLUMNS:
        clean_dict[g] = float(student_dict.get(g, 65.0))
    for a in APTITUDE_COLUMNS:
        clean_dict[a] = float(student_dict.get(a, 65.0))
...
def predict_top_careers(student_dict: dict, selected_field: str, top_n: int = 5, model_dir: str = None) -> pd.DataFrame:
    ...
    top_df["Probability"] = top_df["Final_Score"].round(4)
    return top_df
